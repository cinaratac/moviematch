...
class ChatService {
  ...
  Future<void> ensureChat(String chatId, String uidA, String uidB) async {
    ...
    final partsAny = (snap.data()?['participants'] as List?) ?? const [];
    final parts = partsAny.map((e) => e.toString()).toSet();
    if (parts.contains(uidA) && parts.contains(uidB)) {
  ...
  Future<void> markAsRead(String chatId, String uid) async {
    final chatRef = _fs.collection('chats').doc(chatId);
    // Get chat doc cache-first
    var chatSnap = await chatRef.get(const GetOptions(source: Source.cache));
    if (!chatSnap.exists) {
      chatSnap = await chatRef.get(const GetOptions(source: Source.server));
    }
    final data = chatSnap.data();
    if (data == null) return;
    final counts = (data['unreadCounts'] as Map<String, dynamic>?) ?? const {};
    final myUnreadRaw = counts[uid];
    final myUnread = (myUnreadRaw is num) ? myUnreadRaw.toInt() : 0;
    final lastAuthor =
        (data['lastMessageAuthorId'] ?? data['lastAuthorId'] ?? '') as String?;
    final lastMsgAtTs = data['lastMessageAt'] as Timestamp?;
    // If there is nothing to mark or last message is mine, skip writes
    if (myUnread <= 0 || (lastAuthor == uid)) {
      return;
    }
    ...
    batch.set(readRef, {
      ...
      if (lastMsgAtTs != null) 'lastSeenMessageAt': lastMsgAtTs,
      'updatedAt': FieldValue.serverTimestamp(),
    }, SetOptions(merge: true));
    // Mirror aggregate into chat root
    batch.set(chatRef, {
      'reads': {
        uid: {
          'lastReadAt': FieldValue.serverTimestamp(),
    ...
    // Reset unread counter only if it is > 0 (we already checked)
    batch.update(chatRef, {'unreadCounts.$uid': 0});
    await batch.commit();
  }
