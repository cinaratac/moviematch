...
// Az önce oluşturduğumuz yeni dosyaları çağırıyoruz
...
class _MovieDetailScreenState extends State<MovieDetailScreen> {
  ...
  String get _rating => _movieData != null ? (_movieData!['vote_average'] as num).toStringAsFixed(1) : '-';
    ...
    return Scaffold(
      ...
      body: Stack(
        children: [
          ...
          else if (_hasError || _movieData == null) Center(child: Text("Detaylar yüklenemedi", style: TextStyle(color: textColor)))
          else
            SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                      ...
                      children: [
                        Hero(
                          ...
                          child: ClipRRect(
                            ...
                            child: CachedNetworkImage(imageUrl: widget.posterUrl ?? '', width: 140, height: 210, fit: BoxFit.cover),
                          ),
                        ),
                        ...
                        Expanded(
                          child: Column(
                            crossAxisAlignment: CrossAxisAlignment.start,
                            children: [
                              Text(_movieData!['title'], style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold, color: textColor, height: 1.2)),
                              ...
                              GestureDetector(
                                onTap: () {
                                  ...
                                  if (director != null && director['id'] != null) {
                                    Navigator.push(context, MaterialPageRoute(builder: (_) => DirectorScreen(
                                      directorId: director['id'] as int,
                                      directorName: director['name'] ?? 'Bilinmiyor',
                                    )));
                                  } else {
                                    ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Yönetmen bilgisi bulunamadı.')));
                                  }
                                },
                                child: Column(
                                  crossAxisAlignment: CrossAxisAlignment.start,
                                  children: [
                                    Text("Yönetmen", style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: textColor.withOpacity(0.5))),
                                    Text(_director, style: TextStyle(fontSize: 15, fontWeight: FontWeight.w600, color: textColor, decoration: TextDecoration.underline, decorationColor: textColor.withOpacity(0.3))),
                                  ],
                                ),
                              ),
                            ],
                  ...
                  Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 8),
                        Text(_movieData!['overview'] ?? 'Özet bulunamadı.', style: TextStyle(fontSize: 15, color: textColor.withOpacity(0.8), height: 1.6)),
                      ],
                    ),
                  ),
                  const SizedBox(height: 30),
                    ...
                    child: ListView.separated(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        return GestureDetector(
                          onTap: () {
                            Navigator.push(context, MaterialPageRoute(builder: (_) => ActorScreen(actorId: actor['id'], actorName: actor['name'])));
                          },
                          child: Column(
                            children: [
                              CircleAvatar(
                                radius: 35, backgroundColor: Colors.grey.shade800,
                                backgroundImage: actor['profile_path'] != null ? NetworkImage('https://image.tmdb.org/t/p/w200${actor['profile_path']}') : null,
                                child: actor['profile_path'] == null ? const Icon(Icons.person) : null,
                              ),
                              const SizedBox(height: 8),
                              SizedBox(
                                width: 80,
                                child: Text(actor['name'], maxLines: 2, textAlign: TextAlign.center, overflow: TextOverflow.ellipsis, style: TextStyle(fontSize: 11, color: textColor.withOpacity(0.9))),
                              ),
                            ],
                          ),
                        );
                  ...
                  const SizedBox(height: 30),
                  // KESİN ÇÖZÜM: Yorumlar artık tamamen bağımsız!
                  MovieReviewSection(
                    tmdbId: widget.tmdbId,
                    movieData: _movieData!,
                    posterUrl: widget.posterUrl,
