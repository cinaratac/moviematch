...
class _PassesListBodyState extends State<PassesListBody>
  ...
  Widget build(BuildContext context) {
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      return const Center(child: CircularProgressIndicator());
            ...
            return ListView.builder( // separated yerine builder yapıyoruz
              key: _listKey,
              padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 20),
              itemCount: items.length,
              // separatorBuilder'ı tamamen sildik
              itemBuilder: (context, i) {
                final row = items[i];
                return FutureBuilder<_UserLite>(
                  future: _getUserLite(row.otherUid),
                  builder: (context, userSnap) {
                    // YENİ KONTROL: Username yoksa veya hata oluştuysa satırı tamamen gizle
                    if (userSnap.hasError) {
                      return const SizedBox.shrink(); // Ekranda yer kaplamaz
                    }
                    
                    final lite = userSnap.data;
                    
                    // Sadece veri hazırsa kartı göster
  ...
  Future<_UserLite> _fetchUserLiteFromDb(String uid) async {
   try {
      ...
      if (!doc.exists) throw Exception('Doküman bulunamadı'); // Hata fırlat
      
      final m = doc.data()!;
      final username = (m['username'] ?? '') as String;

      // KRİTİK KONTROL: Username boşsa bu kullanıcıyı listeleme
      if (username.trim().isEmpty) {
        throw Exception('Username eksik, listelenmeyecek.');
      }
    ...
    return _UserLite(title: username, photoURL: photoURL, age: age);
    } catch (e) {
      // Hata fırlatıyoruz ki FutureBuilder bu kullanıcıyı gizleyebilsin
      rethrow;
    }
  }
}
