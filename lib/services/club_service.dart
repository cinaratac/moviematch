...
class ClubService {
  ...
  Future<void> createClub({
    ...
    final chatRef = _db.collection('chats').doc(clubRef.id); // Kulüp ID = Chat ID
    ...
    final clubData = {
      ...
      'ownerId': user.uid,
      'createdAt': now,
      'members': [user.uid], // Kurucu otomatik üyedir
      'admins': [user.uid],  // Kurucu otomatik yöneticidir (YENİ)
      'pendingRequests': [],
    };
  ...
  Future<void> joinClub(String clubId, bool isPrivate) async {
    ...
    if (isPrivate) {
      ...
      batch.update(clubRef, {'members': FieldValue.arrayUnion([uid])});
      batch.update(chatRef, {'participants': FieldValue.arrayUnion([uid])});
      await batch.commit();
    }
  }
